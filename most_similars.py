...
#    with open("rank_XylR","w") as f, open(GUs_andContext) as f2:
#             GUs=[line.strip().split("\t") for line in f2.readlines()]
#             f.write("# The most similars to: '%s'\n\n" % (" ".join(GUs[179])))
#             j=0
#             for i, s in m179:
#                 f.write("%d\t%f\t%s\n" % (i,s, "\t".join(GUs[i])))
#                 if j<20:
...
#                 else:
#                     break
#             j=0; f.write("\n# The less similars to: '%s'\n\n" % (" ".join(GUs[179])))
#             for i, s in reversed(m179):
#                 f.write("%d\t%f\t%s\n" % (i,s, "\t".join(GUs[i])))
#                 if j<20:
#                     j +=1
#                     continue
...
#    with open("rank_AraC","w") as f, open("/almac/ignacio/data/GUsDany/corpus/GUs_textform.txt") as f2:
#             GUs=[line.strip().split("\t") for line in f2.readlines()]
#             f.write("# The most similars to: '%s'\n\n" % (" ".join(GUs[14])))
#             j=0
#             for i, s in m14:
